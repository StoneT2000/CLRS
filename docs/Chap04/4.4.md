...
- We guess $T(n) \le cn^2 - dn$,
    ...
    \end{aligned}
    $$
    The last step holds provided that
    $$
    \begin{aligned}
    ...
    \end{aligned}
    $$
    For large $n$, it would then imply $d \ge 8c + 1$
    
    This is entirely possible and we can determine boundary conditions as well.
...
- We notice that for $n \le 8 $, $T(n)$ isn't exactly going to be $\le$ than $cn^2 - (8c+1)n=cn^2$ assuming positive runtimes.
    ...
    So for all $n \ge n_0 = 12$, and for $c \ge 472/48$, we bound $T(n) \le cn^2$, proving that $T(n) = O(n^2)$
